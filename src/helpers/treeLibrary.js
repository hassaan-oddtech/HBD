...
export class Point {
  ...
  add(o) { let p = this.clone(); p.x += o.x; p.y += o.y; return p; }
  sub(o) { let p = this.clone(); p.x -= o.x; p.y -= o.y; return p; }
  div(n) { let p = this.clone(); p.x /= n; p.y /= n; return p; }
  mul(n) { let p = this.clone(); p.x *= n; p.y *= n; return p; }
}
...
class Seed {
  constructor(tree, point, scale, color) {
    this.tree = tree;
    var scaleOpt = scale || 1;
    var colorOpt = color || "#FFC0CB";
    this.heart = { point: point, scale: scaleOpt, color: colorOpt, figure: new Heart() };
    this.cirle = { point: point, scale: scaleOpt, color: colorOpt, radius: 5 };
  }
  draw() {
    this.drawHeart();
    this.drawText();
...
// clear() {
//     var ctx = this.tree.ctx, cirle = this.cirle, point = cirle.point, scale = cirle.scale;
//     // Fix: Enlarged clearing area for text
//     ctx.clearRect(point.x - 50, point.y - 50, 400 * scale, 200 * scale);
//   }
  hover(x, y) {
    var ctx = this.tree.ctx;
    // We only check if the pixel at mouse is non-transparent
...
class Branch {
  ...
  draw(p) {
    ...
    ctx.closePath();
    ctx.fill();
    ctx.restore(); // Fixed: Originally commented out
  }
}
